...
interface UserControllerDocs {
    @Operation(
        ...
        parameters = [Parameter(name = "Authorization", description = "액세스 토큰", required = true)],
        responses = [ApiResponse(
            responseCode = "200",
            description = "조회 성공",
            content = [Content(mediaType = "application/json", schema = Schema(implementation = MyPageRes::class))]
        )]
    )
    fun getMyPage(@RequestHeader("Authorization") bearerToken: String): ResponseEntity<*>
    ...
    @Operation(
        ...
        responses = [ApiResponse(
            responseCode = "200",
            description = "수정 성공",
            content = [Content(mediaType = "application/json", schema = Schema(implementation = ProfileRes::class))]
        )]
    )
    fun updateProfile(
        @RequestHeader("Authorization") bearerToken: String,
        @RequestPart("dto") reqDto: ProfileUpdateReq,
