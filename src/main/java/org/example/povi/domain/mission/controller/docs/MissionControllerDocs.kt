...
interface MissionControllerDocs {
    ...
    fun getTodayMissions(
        @Parameter(description = "JWT 인증 토큰") @RequestHeader("Authorization") bearerToken: String
    ...
    fun updateStatus(
        @Parameter(description = "JWT 인증 토큰") @RequestHeader("Authorization") bearerToken: String,
        @Parameter(description = "유저 미션 ID") @PathVariable userMissionId: Long,
