...
interface CommunityControllerDocs {
    ...
    fun createPost(
            ...
            photos: List<MultipartFile?>?): ResponseEntity<PostCreateResponse>
    ...
    fun getMyPostList(
            @RequestHeader("Authorization") bearerToken: String?,
            @PageableDefault(size = 4, sort = ["createdAt,desc"]) pageable: Pageable?): ResponseEntity<Page<PostListResponse?>?>?


    @Operation(summary = "커뮤니티 글 상세 조회", description = "특정 커뮤니티 글의 상세 내용을 조회합니다.")
    @ApiResponses(value = [ApiResponse(responseCode = "200", description = "조회 성공", content = [Content(schema = Schema(implementation = PostDetailResponse::class))]), ApiResponse(responseCode = "404", description = "게시글 없음")])
    @GetMapping("/{postId}")
    ...
    fun deleteComment(
            @RequestHeader("Authorization") bearerToken: String?,
            @PathVariable commentId: Long?): ResponseEntity<CommentDeleteResponse?>?
    ...
    fun getMyComments(
            @RequestHeader("Authorization") bearerToken: String?,
            @PageableDefault(size = 2, sort = ["createdAt,desc"]) pageable: Pageable?): ResponseEntity<Page<CommentListResponse?>?>?
    ...
    fun getMyLikedPosts(
            @RequestHeader("Authorization") bearerToken: String?,
            @PageableDefault(size = 2, sort = ["createdAt,desc"]) pageable: Pageable?): ResponseEntity<Page<LikeListResponse?>?>?
    ...
    fun getMyBookmarks(
            @RequestHeader("Authorization") bearerToken: String?,
            @PageableDefault(size = 4, sort = ["createdAt,desc"]) pageable: Pageable?): ResponseEntity<Page<BookmarkListResponse?>?>?
}
